...
if __name__ == "__main__":
    ...
    value = 2**0.5 / 2
    R_targ = tf.convert_to_tensor(
        [[value, value, 0], [-value, value, 0], [0, 0, 1]], dtype=tf.float32
    )
    T_targ = tf.expand_dims(tf.convert_to_tensor([0, 0, -2.5], dtype=tf.float32), -1)
    K_src = tf.convert_to_tensor(
        [[2400, 0, 959.5], [0, 2400, 539.5], [0, 0, 1]], dtype=tf.float32
    )
    K_targ = tf.convert_to_tensor(
        [[3200, 0, 959.5], [0, 3200, 539.5], [0, 0, 1]], dtype=tf.float32
    )
    ...
    for d in [2.5]:
        ...
        for i in range(1000):
            with tf.GradientTape() as tape:
                warped_result = warp_imgs(
                    imgs, pixel_coords_targ, k_s, k_t, rot, t, n_trans, a
                )
